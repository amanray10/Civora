...
exports.updateStatus = async (req,res) => {
  ...
  try {
    ...
    if (
      ['department', 'admin'].includes(req.user.role) &&
      existing.departmentId !== req.user.departmentId
    ){
      return res.status(403).json({
        error: "This complaint belongs to another department."
      });
    }
    ...
    let dupes = await prisma.complaint.findMany({
    ...
    if (
      ['Resolved', 'Closed', 'In Progress'].includes(status) &&
      dupes.length
    ){
    ...
    if (existing.user?.email) {
      ...
      await sendEmail(
        ...
        `
        <h2>Civora Complaint Update</h2>

        <p>Hello ${existing.user.name || "User"},</p>
        ...
        <p>
        <b>Remark:</b> ${note || "No remark provided"}
        </p>
    ...
    res.status(500).json({
      error:"Error occurred while updating status."
    });
...
exports.assignDepartment = async (req,res) => {
  ...
  try {
    ...
    getIO()?.to(`dept:${departmentId}`).emit(
      'complaint:new',
      { id: id }
    );
    ...
    res.status(500).json({
      error: "Error occurred while assigning department."
    });
